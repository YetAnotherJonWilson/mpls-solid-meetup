...
export function isSuccessfulStatusCode(number) {
  return Math.floor(statusCode / 100) === 2;
}

export async function createSolidContainer(url, name) {
  console.log('We are creating container for the app');
...
export async function performLogin() {
  ...
  if (!loginURLhref) return null;

  login({
    oidcIssuer: loginURLhref,
    redirectUrl: 'http://localhost:3000',
    clientName: 'Minneapolis Solid Project',
  });
...
function getLogInURL() {
  const url = prompt(
    'What is your Solid login URL? Example: https://timeasolidweb.solidweb.org/'
  );
